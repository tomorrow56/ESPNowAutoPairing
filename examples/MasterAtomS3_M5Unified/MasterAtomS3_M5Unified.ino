...
// pairing_message_t はライブラリ側で定義済み（dataは8バイト）
...
void setup() {
  ...
  espNow.begin();
  espNow.setUserRecvCallback(OnDataRecvUser); // ライブラリ内部処理の後に呼ばれる

  M5.Lcd.setTextSize(1);
  M5.Lcd.setTextColor(BLUE, BLACK);
  M5.Lcd.fillScreen(BLACK);
  M5.Lcd.setCursor(0, 5);
  M5.Lcd.print("ESP-NOW Master Mode");
  M5.Lcd.setTextColor(WHITE, BLACK);
  M5.Lcd.setCursor(0, 20);
  M5.Lcd.print("Hold BtnA to Pairing");
}
...
void loop() {
  ...
  if (espNow.isPaired()) {
    ...
    // マスターのMACアドレスをデータバイトとして送信（8バイト枠に収める）
    ...
    if (millis() - lastDataSendTime > 2000) { // 2秒ごとにデータを送信
      ...
      M5.Lcd.setCursor(5, 45); // 送信メッセージ表示位置
  ...
  if (millis() - lastSendTime > 1000) {
    pairing_message_t requestMsg;
    requestMsg.type = PAIR_REQUEST;
    uint8_t myMac[6];
    WiFi.macAddress(myMac);
    memcpy(requestMsg.mac, myMac, 6);
    uint8_t broadcastAddress[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    esp_now_send(broadcastAddress, (uint8_t*)&requestMsg, sizeof(requestMsg));
    Serial.println("Sent PAIR_REQUEST");
    lastSendTime = millis();
  }
